...
const NavigationItem = ({item, isActive, isCollapsed, onClick}) =>{
  ...
  return <button onClick={() => onClick(item.id)} className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${
    isActive 
...
const DashboardLayout = ({activeMenu, children}) => {
  ...
  const [ profileDropdownOpen, setProfilDropdownOpen] = useState(false);
    ...
    <div className='flex h-screen bg-gray-50'>
      ...
      <div className={` fixed inset-y-0 left-0 z-50 transition-transform duration-300 transform ${
        ...
        ? sidebarOpen 
          ? "translate-x-0" 
          :"-translate-x-full"
        : "translate-x-0"} ${sidebarCollapsed ? "w-16" : "w-64"} bg-white border-r border-gray-200`}>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${ isMobile ? "ml-0" : sidebarCollapsed ? "ml-16" : "ml-64"}`}>
        ...
        {/* top navbar  */}
        <header className='bg-white/80 backdrop-blur-sm border-b border-gray-200 h-16 flex items-center justify-between sticky top-0 z-30'>
          <div className='flex items-center space-x-4'>
            ...
            <div>
              <h1 className='text-base font-semibold text-gray-900 md:ml-2 lg:ml-4'>
                Welcome Back!
              </h1>
              <p className='text-sm text-gray-600 hidden sm:block md:ml-2 lg:ml-4'> Here's what's happening with your jobs today.</p>
            </div>
          </div>
          ...
          <div className='flex items-center space-x-3'>
            ...
            <ProfileDropdown
              ...
              onToggle = {(e)=>{
                e.stopPropagation();
                setProfilDropdownOpen(!profileDropdownOpen);              
              }}
              avatar={user?.avatar || ""}
